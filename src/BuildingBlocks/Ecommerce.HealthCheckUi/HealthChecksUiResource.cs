...
namespace Ecommerce.HealthCheckUi;
    ...
    )
    {
        ...
        foreach (var healthChecksUiResource in healthChecksUiResources)
        {
            foreach (var monitoredProject in healthChecksUiResource.MonitoredProjects)
            {
                ...
                {
                    int? targetPort = executionContext.IsPublishMode
                        ? DefaultHealthChecksPort
                        : null;
                    project.WithHttpEndpoint(
                        targetPort: targetPort,
                        name: monitoredProject.EndpointName
                    );
                    Debug.Assert(
                ...
                project.WithEnvironment(context =>
                {
                    ...
                    {
                        if (
                            !context.EnvironmentVariables.ContainsKey(AspnetcoreHttpPorts)
                        ...
                        if (
                            !context.EnvironmentVariables.ContainsKey(AspnetcoreHttpsPorts)
